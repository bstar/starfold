...
/// Preserve OSC 72 responses as a single event. The protocol limits each
/// escape's payload to 4096 bytes, so a larger unfinished response is invalid.
fn parse_osc72(buffer: &[u8]) -> io::Result<Option<InternalEvent>> {
    const MAX_ESCAPE: usize = 8192;
    if buffer.len() > MAX_ESCAPE {
    ...
    let body = std::str::from_utf8(&buffer[5..buffer.len() - 2])
        .map_err(|_| could_not_parse_event_error())?;
    Ok(Some(InternalEvent::Event(Event::Osc72(body.to_owned()))))
}
...
mod osc72_tests {
    ...
    #[test]
    fn parses_complete_message_and_waits_for_terminator() {
        let data = b"\x1b]72;t=m:x=2:y=3;image/png\x1b\\";
        for end in 1..data.len() {
            assert_eq!(parse_event(&data[..end], true).unwrap(), None);
        }
        assert_eq!(
            parse_event(data, false).unwrap(),
            Some(InternalEvent::Event(Event::Osc72(
                "t=m:x=2:y=3;image/png".to_owned()
            )))
        );
    }

    #[test]
    fn ignores_other_osc_without_exposing_payload_as_keys() {
        assert!(parse_event(b"\x1b]0;title\x1b\\", false).is_err());
    }
}

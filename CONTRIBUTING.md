...
linker cannot read, and its symbols come back undefined. The release profile
...
The icon is `packaging/starfold.svg`, and `packaging/starfold.png` is that file
at 256x256. If you change one, regenerate the other:

```sh
nix shell nixpkgs#librsvg -c rsvg-convert -w 256 -h 256 \
  -o packaging/starfold.png packaging/starfold.svg
```
